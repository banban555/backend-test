...
app.post('/signin', async (req, res) => {
  try {
    ...
    const isMatched = await userInfo.comparePassword(req.body.password);
    ...
    // 토큰을 쿠키에 저장
    res.cookie('x_auth', token).status(200).send({
      success: true,
      userId: userInfo._id,
    });
  } catch (error) {
...
app.post('/application/add', auth, async (req, res) => {
  try {
    ...
    if (result.modifiedCount === 0) {
      return res.status(400).json({ success: false, message: '강의 추가에 실패했습니다.' });
    }

    res.status(200).json({ success: true, message: '강의 추가에 성공했습니다.' });
  } catch (error) {
    console.error(error);
    res.status(500).json({ success: false, error: 'Internal server error' });
  }
...
app.delete('/application/delete', auth, async (req, res) => {
  try {
    ...
    if (result.modifiedCount === 0) {
      return res.status(400).json({ success: false, message: '강의 삭제에 실패했습니다.' });
    }

    res.status(200).json({ success: true, message: '강의 삭제에 성공했습니다.' });
  } catch (error) {
    console.error(error);
    res.status(500).json({ success: false, error: 'Internal server error' });
  }
